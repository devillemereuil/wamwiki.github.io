...
{{% alert title="Non-Gaussian traits" color="primary" %}}
When traits are non-Gaussian (i.e. we cannot assume the residual of a linear model will follow a normal distribution), like survival or (often) fecundity, we need to use models that extend the principals of the animal models beyond the normal distribution.
